class ColorPalette {
  constructor() {
    ...
    this.colorAdjustSliderCloseBtns = document.querySelectorAll(
      ".color-adjust-slider-close-btn"
    );
    ...
    this.paletteLibraryModalContainer = document.querySelector(
      ".palette-library-modal-container"
    );
    this.paletteLibraryModalBody = document.querySelector(
      ".palette-library-modal-body"
    );
    ...
    this.savePaletteModalContainer = document.querySelector(
      ".save-palette-modal-container"
    );
    ...
    this.savePaletteCloseBtn = document.querySelector(
      ".save-palette-close-btn"
    );
    this.savePaletteSubmitBtn = document.querySelector(
      ".save-palette-submit-btn"
    );
  }
  ...
  BgManipualtion(event) {
    ...
    const changedColor = chroma(currentColorHexText)
      .set("hsl.h", currentHue.value)
      .set("hsl.s", currentSaturation.value)
      .set("hsl.l", currentBrightness.value);
  ...
  BgTextIconUpdate(index) {
    ...
    const colorToggleIcons = selectedColor.querySelectorAll(
      ...
      this.paletteNameBundle.forEach((bundleName, serialNumber) => {
        ...
        deletePalette.addEventListener("click", () => {
          ...
          localStorage.setItem(
            "palette-keys",
            JSON.stringify(this.paletteNameBundle)
          );
  ...
  SavePaletteOpen() {
    ...
    for (let i = 0; i < saveColorColumns.length; i++) {
      saveColorColumns[i].style.backgroundColor =
        this.initialGeneratedColors[i];
    }
  ...
  SavePaletteToLibrary() {
    ...
    if (paletteName !== "") {
      ...
      localStorage.setItem(
        paletteName,
        JSON.stringify(this.initialGeneratedColors)
      );
...
presentColorPalette.paletteLibraryCloseBtn.addEventListener(
  "mouseenter",
  (e) => {
